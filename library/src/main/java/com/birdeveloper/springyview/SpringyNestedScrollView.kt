...
import com.birdeveloper.springyview.effect.SpringyEdgeEffect
...
/**
 * It is derived from NestedScrollView.
 */

class SpringyNestedScrollView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0)
    ...
    ScrollingView
{
    ...
    {
        fun onScrollChange(v: SpringyNestedScrollView?, scrollX: Int, scrollY: Int, oldScrollX: Int, oldScrollY: Int)
    }

    private var mLastScroll: Long = 0
    private val mTempRect = Rect()
    private var mScroller: OverScroller? = null
    private var mEdgeGlowTop: SpringyEdgeEffect? = null
    private var mEdgeGlowBottom: SpringyEdgeEffect? = null
    ...
    override fun dispatchNestedScroll(dxConsumed: Int, dyConsumed: Int, dxUnconsumed: Int, dyUnconsumed: Int, offsetInWindow: IntArray?, type: Int): Boolean = mChildHelper.dispatchNestedScroll(dxConsumed, dyConsumed, dxUnconsumed, dyUnconsumed, offsetInWindow, type)

    override fun dispatchNestedPreScroll(dx: Int, dy: Int, consumed: IntArray?, offsetInWindow: IntArray?, type: Int): Boolean = mChildHelper.dispatchNestedPreScroll(dx, dy, consumed, offsetInWindow, type)
    ...
    override fun dispatchNestedScroll(dxConsumed: Int, dyConsumed: Int, dxUnconsumed: Int, dyUnconsumed: Int, offsetInWindow: IntArray?): Boolean = mChildHelper.dispatchNestedScroll(dxConsumed, dyConsumed, dxUnconsumed, dyUnconsumed, offsetInWindow)

    override fun dispatchNestedPreScroll(dx: Int, dy: Int, consumed: IntArray?, offsetInWindow: IntArray?): Boolean = dispatchNestedPreScroll(dx, dy, consumed, offsetInWindow, ViewCompat.TYPE_TOUCH)

    override fun dispatchNestedFling(velocityX: Float, velocityY: Float, consumed: Boolean): Boolean = mChildHelper.dispatchNestedFling(velocityX, velocityY, consumed)
    ...
    // NestedScrollingParent3
    override fun onNestedScroll(target: View, dxConsumed: Int, dyConsumed: Int, dxUnconsumed: Int, dyUnconsumed: Int, type: Int, consumed: IntArray) = onNestedScrollInternal(dyUnconsumed, type, consumed)
    ...
    private fun ensureGlows()
    {
        if (overScrollMode != OVER_SCROLL_NEVER)
        {
            ...
            {
                val context = context
                mEdgeGlowTop = SpringyEdgeEffect(context, spring, this, EdgeEffectFactory.DIRECTION_TOP, flingAnimationSize, overscrollAnimationSize)
                mEdgeGlowBottom = SpringyEdgeEffect(context, spring, this, EdgeEffectFactory.DIRECTION_BOTTOM, flingAnimationSize, overscrollAnimationSize)
            }
        }
        else
        {
    ...
    internal class AccessibilityDelegate : AccessibilityDelegateCompat()
    {
        override fun performAccessibilityAction(host: View, action: Int, arguments: Bundle): Boolean
        {
            ...
            val nsvHost = host as SpringyNestedScrollView
    ...
    companion object
    {
        ...
        private const val TAG = "SpringyNestedScrollView"
    ...
    init
    {
        ...
        context.theme.obtainStyledAttributes(attrs, R.styleable.SpringyNestedScrollView, 0, 0)
            .apply{
                overscrollAnimationSize = getFloat(R.styleable.SpringyNestedScrollView_scrollAnimationSize, 0.5f)
                flingAnimationSize = getFloat(R.styleable.SpringyNestedScrollView_flingAnimationSize, 0.5f)
                when (getInt(R.styleable.SpringyNestedScrollView_dampingScoll, 0))
                {
                ...
                when (getInt(R.styleable.SpringyNestedScrollView_stiffnessScroll, 1))
                {
